...
- **Old Server:** 10.27.20.171 (or any existing Wazuh installation)
- **New Server:** 10.27.20.181 (or your target server)
...
```bash
# Transfer rules
scp root@10.27.20.171:/var/ossec/etc/rules/custom_windows_security_rules.xml \
    root@10.27.20.181:/root/migration-temp/

scp root@10.27.20.171:/var/ossec/etc/rules/local_rules_override.xml \
    root@10.27.20.181:/root/migration-temp/

# Transfer integration script
scp root@10.27.20.171:/var/ossec/integrations/custom-teams-summary.py \
    root@10.27.20.181:/root/migration-temp/

# Transfer cache (optional, to preserve alert history)
scp root@10.27.20.171:/var/ossec/logs/teams_alerts_cache.pkl \
    root@10.27.20.181:/root/migration-temp/ 2>/dev/null || echo "No cache to transfer"
```
...
3. **Update agents (if they were changed):**
   - Change `<address>` back to `10.27.20.171` in agent ossec.conf
   - Restart agents
...
```bash
echo "Migrated to 10.27.20.181 on $(date)" >> /root/wazuh_changelog.txt
```
...
```bash
# Update A record wazuh.yourdomain.com to point to 10.27.20.181
```
